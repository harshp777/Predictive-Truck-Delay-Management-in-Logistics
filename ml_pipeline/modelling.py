...
def train_logistic_model(X_train, y_train, X_valid, y_valid, X_test, y_test, class_weights, project_config):
    ...
    with wandb.init(project=PROJECT_NAME) as run:
        ...
        #shutil.copy("E://Artificial Intelligence//Projects//Predictive-Truck-Delay-Management-in-Logistics//models//log-truck-model.pkl", os.path.join(wandb.run.dir, "log-truck-model.pkl"))
        
        # models\log-truck-model.pkl
        # Define the relative path to the source file
        source_path = os.path.join("models", "log-truck-model.pkl")

        # Define the destination path using wandb.run.dir
        destination_path = os.path.join(wandb.run.dir,"log-truck-model.pkl")
...
def train_random_forest(X_train, y_train, X_valid, y_valid, X_test, y_test, class_weights, project_config):
    ...
    with wandb.init(project=PROJECT_NAME) as run:
        ...
        #shutil.copy("E://Artificial Intelligence//Projects//Predictive-Truck-Delay-Management-in-Logistics//models//randomf-truck-model.pkl", os.path.join(wandb.run.dir, "randomf-truck-model.pkl"))
        
        # Define the relative path to the source file
        source_path = os.path.join("models", "randomf-truck-model.pkl")

        # Define the destination path using wandb.run.dir
        destination_path = os.path.join(wandb.run.dir, "randomf-truck-model.pkl")
...
def train_xgb_model(X_train, y_train, X_valid, y_valid, X_test, y_test, project_config):
    ...
    with wandb.init(project=PROJECT_NAME) as run:
        ...
        #shutil.copy("E://Artificial Intelligence//Projects//Predictive-Truck-Delay-Management-in-Logistics//models//xgb-truck-model.pkl", os.path.join(wandb.run.dir, "xgb-truck-model.pkl"))
        
        
        # Define the relative path to the source file
        source_path = os.path.join("models", "xgb-truck-model.pkl")

        # Define the destination path using wandb.run.dir
        destination_path = os.path.join(wandb.run.dir, "xgb-truck-model.pkl")
